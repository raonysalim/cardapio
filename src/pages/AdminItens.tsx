...
export default function AdminItens(){
    ...
    return (
      ...
      <Title/>
      <EditMenu/>
      <h1 className={'menuTitle'}>{itens[0].category.name}</h1>
      {itens.map((i)=>{
      return (
      <div key={i.id}> 
      <span  className="selectCategory">
        ...
        <button className="btnEdit" onClick={()=>{
            navigate(`/admin/edit/itens/${i.id}?name=${i.name}&description=${i.description}&price=${i.price}&category=${i.categoryId}&image=${i.image? true : ''}`)
